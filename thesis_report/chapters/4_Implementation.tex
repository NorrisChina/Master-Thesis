...
This half-width defines a \emph{bounded-support threshold}: choosing $R\ge a$ guarantees $|e|\le R$ for all samples under the Uniform model and therefore makes tolerance violations impossible by construction.
...
\begin{figure}[H]
    ...
    \caption{Computational scalability analysis showing decoder latency as a function of quantization resolution $\Delta$. The red curve represents the bounded-support configuration ($R=30$). At the high-precision limit ($\Delta=0.01$), latency reaches a prohibitive 12.8 ms. However, at the selected operating point ($\Delta=0.1$, indicated by the vertical dotted line), the latency drops to approximately 1.3 ms, balancing physical coverage with real-time computational feasibility.}
