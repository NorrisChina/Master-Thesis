...
\end{equation}
The generation of a verifier $v$ using a random seed $r$ is geometrically equivalent to \emph{sampling this curve} at the coordinate $x=r$:
\begin{equation}
    v = P_S(r) \pmod q
\end{equation}
...
\end{equation}
Let $\Delta(x) = P_S(x) - P_{\hat{S}}(x)$ be the difference polynomial. Since $S \neq \hat{S}$, $\Delta(x)$ is a non-zero polynomial with degree at most $k-1$. By the \emph{Fundamental Theorem of Algebra}, a non-zero polynomial of degree $d$ can have at most $d$ roots. 
