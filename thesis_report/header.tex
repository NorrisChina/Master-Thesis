...
%	ngerman,
	...
	cdfont=false,					% Do not use the TUD font (not installed in container)
...
]{tudscrreprt} 				% use TU Dresden KOMA-Script report class

% better hyphenation rules (must be loaded after the class)
\usepackage{hyphsubst}
% Allow inclusion of newer PDF versions (e.g., 1.7) without warnings
\pdfminorversion=7
\usepackage{scrhack}	% includes koma script patches for other packages

% Use non-bold chapter headings (chapters 1--6) for a more conventional thesis look.
% Keep sizes controlled by KOMA/TUD-Script; only remove bold weight.
...
% \usepackage[default,scale=0.95]{opensans}	% font for the CD
\usepackage[utf8]{inputenc} % UTF-8 Support
\usepackage{lmodern} % scalable Latin Modern fonts
\usepackage[T1]{fontenc} % T1 Fonts for font encoding
% Avoid repeated font warnings for lmss/sbx (tudscr requests sbx; map to semi-bold).
% The lmss font definition file is loaded lazily; force-load it so our override sticks.
\makeatletter
\InputIfFileExists{t1lmss.fd}{}{}
\makeatother
\DeclareFontShape{T1}{lmss}{sb}{n}{<->ssub*lmss/b/n}{}
\DeclareFontShape{T1}{lmss}{sbx}{n}{<->ssub*lmss/b/n}{}
...
\usepackage{float}		% Provides the [H] float specifier for exact placement
...
%
% ===============================================
% Keywords helper (used in abstract)
% ===============================================
...
\newglossarystyle{acrotabularx}{%
	...
	\renewcommand*{\glossentry}[2]{%
		...
		\hspace{1em}\glsentrydesc{##1}\tabularnewline
	}
}

% Acronym table style with page numbers.
% Uses a narrow, wrapped rightmost column to avoid overfull boxes.
\newlength{\acropagecolwidth}
\newglossarystyle{acrotabularxpages}{%
	\renewenvironment{theglossary}{%
		\par\setlength{\parindent}{0pt}\noindent%
		\small%
		\setlength{\hfuzz}{12pt}% suppress minor overfull warnings in long acronym lists
		\sloppy\emergencystretch=1em%
		\renewcommand*{\delimN}{,\allowbreak\space}% allow breaks in page lists
		\renewcommand*{\delimR}{--\allowbreak}% allow breaks after page ranges
		% Make the location list breakable inside the table cell.
		\renewcommand*{\glossaryentrynumbers}[1]{\hspace{0pt}####1}%
		\setlength{\acropagecolwidth}{0.36\linewidth}%
		\setlength{\tabcolsep}{0.3em}% readable separation between columns
		\tabularx{\linewidth}{@{}lX>{\raggedleft\arraybackslash}p{\acropagecolwidth}@{}}%
	}{%
		\endtabularx\par\bigskip%
	}%
	\renewcommand*{\glossaryheader}{}%
	...
	\renewcommand*{\glossentry}[2]{%
		\glsentryitem{##1}% Entry number if required
		\glstarget{##1}{\sffamily\bfseries\glossentryname{##1}} &
		\hspace{1em}\glsentrydesc{##1} &
		{\scriptsize\hspace{0pt}##2}\tabularnewline
	}%
}
% ===============================================
